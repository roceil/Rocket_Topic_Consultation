...
export default function CounselorListCard({ className, counselorName, subtitle, img, description, id }: ICounselorListCardProps) {
  ...
  const startChat = (e: { preventDefault: () => void; }) => {
    e.preventDefault();
    if (identity === 'user') {
      dispatch({
  ...
  return (
    <Link href={`/counselorlist/counselorpage/${id}`} passHref className={`${isBefore} group/image`}>
      {/* 這是圖片 */}
      <Image src={img} alt={counselorName} width={220} height={327} className="h-[270px] w-[41.5789%] bg-primary-tint object-cover lg:h-[327px] lg:w-[220px] group-hover/image:opacity-70" />
      ...
      <div className="flex w-[58.421%] py-3 flex-col justify-center pl-4  pr-[18px] lg:h-[327px] lg:w-[268px] lg:px-6">
        ...
        <p className="mb-3 min-h-[48px] text-sm font-bold text-gray-900 lg:mb-6 lg:text-base">{subtitle}</p>
